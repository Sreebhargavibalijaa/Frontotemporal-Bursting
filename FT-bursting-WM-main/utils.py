...
def read_openneuro(path, subjects):
    ...
    for subject in subjects:
        ...
        # Read trial id info if it exists, otherwise create from performance data
        trial_ids_path = os.path.join(path, "sourcedata", f"{subject}_task-OWM_trialids.tsv")
        if os.path.exists(trial_ids_path):
            trial_ids = pd.read_csv(trial_ids_path, sep = "\t")
        else:
            # Create trial_ids from performance data - assume trials are in order
            trial_ids_data = []
            for channel in performance["channel"].unique():
                n_trials_for_channel = len(performance[performance["channel"] == channel])
                for trial_idx in range(n_trials_for_channel):
                    trial_ids_data.append({"channel": channel, "trial_idx": trial_idx})
            trial_ids = pd.DataFrame(trial_ids_data)
