...
FIX: torch.compile() DISABLED for all three models.
  Root cause: Triton Error [CUDA]: device kernel image is invalid
  -- Triton JIT-compiled kernels are incompatible with the CUDA driver
     on this A30 (SM86 compute capability / driver version mismatch).
  -- torch.compile() is removed; all other A30 optimisations remain.
  -- To re-enable after upgrading PyTorch+Triton to match your driver:
       pip install torch torchvision --index-url https://download.pytorch.org/whl/cu121 --upgrade
     then uncomment the torch.compile() blocks below.
...
print(f"torch.compile: DISABLED (Triton kernel image mismatch on this driver)")
...
# torch.compile() DISABLED -- Triton kernel image invalid on this driver.
# Uncomment after: pip install torch torchvision --index-url .../cu121 --upgrade
# if hasattr(torch, "compile"):
#     model_snn = torch.compile(model_snn)
...
# torch.compile() DISABLED -- see note in Section 6
# if hasattr(torch, "compile"):
#     model_ann = torch.compile(model_ann)
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 8  Ablation: random phase init
# ═══════════════════════════════════════════════════════════════════════════════
...
# torch.compile() DISABLED -- see note in Section 6
# if hasattr(torch, "compile"):
#     model_rand = torch.compile(model_rand)
...
ax = axes[2]
# model_snn is not compiled, so access parameters directly
theta_vals = [p.data.cpu().float().numpy().flatten()
              for n, p in model_snn.named_parameters() if "theta" in n]
if theta_vals:
